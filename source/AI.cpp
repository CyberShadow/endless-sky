...
void AI::UpdateKeys(PlayerInfo &player, Command &clickCommands, bool isActive)
{
	...
	escortsUseAmmo = Preferences::Has(Preferences::ESCORTS_EXPEND_AMMO);
	escortsAreFrugal = Preferences::Has(Preferences::FRUGAL_ESCORTS);
...
void AI::MovePlayer(Ship &ship, const PlayerInfo &player)
{
	...
	AimTurrets(ship, command, !Preferences::Has(Preferences::TURRETS_FOCUS_FIRE));
	if(Preferences::Has(Preferences::AUTOMATIC_FIRING) && !ship.IsBoarding()
	...
	if(Preferences::Has(Preferences::AUTOMATIC_AIMING) && !command.Turn() && !ship.IsBoarding()
			&& (Preferences::Has(Preferences::AUTOMATIC_FIRING) || keyHeld.Has(Command::PRIMARY))
